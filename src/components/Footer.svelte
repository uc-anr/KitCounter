...
<footer>
    <Countdown from="2022-09-30 30:00" format=" H:m:s" zone="Indian" let:remaining >
        <div class="whatever">
    ...
    {:else}
    <button on:click="{()=>dispatch('prevquest')}" disabled style="background-color: rgba(230, 221, 220,0.8);cursor:text; color:black">Previous</button>
    {/if}
    ...
    <button on:click="{()=>dispatch('nextquest')}">Next</button>
        {:else}
        <button on:click="{()=>dispatch('nextquest')}" disabled style="background-color:  rgba(230, 221, 220,0.8);cursor:text; color:black">Next</button>
